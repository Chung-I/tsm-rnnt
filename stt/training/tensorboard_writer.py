...
class AttentionTensorboardWriter(FromParams):
    """
            ...
            for name, param in model.named_parameters():
                ...
                if param.grad is not None:
                    ...
                    if torch.prod(torch.tensor(grad_data.shape)).item() > 0:  # pylint: disable=not-callable
                        self.add_train_scalar(
                            "gradient_mean/" + name, grad_data.mean())
                        self.add_train_scalar(
    ...
    def log_metrics(self,
        ...
        if log_to_console:
            ...
            logger.info(header_template, "Training".rjust(
                name_length + 13), "Validation")
        ...
        for name in metric_names:
            ...
            if log_to_console and val_metric is not None and train_metric is not None:
                logger.info(dual_message_template, name.ljust(
                    name_length), train_metric, val_metric)
            elif log_to_console and val_metric is not None:
                logger.info(no_train_message_template, name.ljust(
                    name_length), "N/A", val_metric)
            elif log_to_console and train_metric is not None:
                logger.info(no_val_message_template, name.ljust(
                    name_length), train_metric, "N/A")
        ...
        if self._histogram_interval is not None:
            ...
            for _, module in model.named_modules():
                ...
                def hook(module_, inputs, outputs):
                    ...
                    log_prefix = 'activation_histogram/{0}'.format(
                        module_.__class__)
