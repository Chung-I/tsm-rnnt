...
class PhnMoChA(Model):
    ...
    def __init__(self,
        ...
        self._logs: Dict[str, Union[Metric, None]] = {
            ...
            "att_loss": Average(),
            "phn_ctc_loss": (Average() if self.phn_ctc_layer else None),
    ...
    def forward(self,  # type: ignore
        ...
        if self._phn_ctc_layer and \
            ...
            phn_ctc_output_dict = self._phn_ctc_layer(source_features, source_lengths, target_tokens)
            output_dict.update({f"phn_ctc_{key}": value for key, value in phn_ctc_output_dict.items()})

        if self._rnnt_layer is not None:
            rnnt_output_dict = self._rnnt_layer(source_features, source_lengths, target_tokens)
            output_dict.update({f"ctc_{key}": value for key, value in rnnt_output_dict.items()})
        if self._ctc_layer is not None:
            ctc_output_dict = self._ctc_layer(source_features, source_lengths, target_tokens)
            output_dict.update({f"rnnt_{key}": value for key, value in ctc_output_dict.items()})

        if target_tokens and self._ctc_layer.loss_ratio < 1.0 and \
            self._target_granularity == self._target_namespace:
            ...
            self._log["att_wer"](output_dict["predictions"], targets)
        ...
        if self.training:
            att_ratio = 1 - self._ctc_layer.loss_ratio - self._rnnt_layer.loss_ratio - \
                self._dep_ratio - self._pos_ratio
    ...
    def _collect_losses(self,
        ...
        if "rnnt_loss" in output_dict:
            loss += ctc * output_dict["rnnt_loss"]
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for key, metric_tracker in self._logs.items():
            ...
            if metric_tracker is not None:
                all_metrics[key] = metric_tracker.get_metric(reset=reset)
        if self._ctc_layer:
            all_metrics.update(self._ctc_layer.get_metrics(reset=reset))
        if self._rnnt_layer:
            all_metrics.update(self._rnnt_layer.get_metrics(reset=reset))
